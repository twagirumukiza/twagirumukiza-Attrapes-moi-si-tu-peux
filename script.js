...
function resolveMove(idx,steps,g=game){const p=g.players[idx];if(!p||!p.alive||g.status!=='playing')return{};const targetIdx=nextAlive(idx,g),target=g.players[targetIdx];const d=distAhead(p.pos,target.pos,g.track);p.pos=(p.pos+steps)%g.track;let eliminated=null;if(steps>=d){target.alive=false;eliminated=targetIdx;const remain=aliveIndices(g).length;if(remain>1)shrink(g.track,TRACK[remain],g);}const remain=aliveIndices(g);if(remain.length===1){g.status='finished';g.turn=remain[0];return{eliminated,winner:g.players[remain[0]]};}g.turn=nextAlive(idx,g);return{eliminated};}
...
function renderBoard(overridePos){
  ...
  game.players.forEach((p,i)=>{
    if(!p.alive)return;
    const pos=(overridePos&&overridePos[i]!==undefined)?overridePos[i]:p.pos;
...
async function performRoll(isAI=false){
  ...
  if(res.eliminated!==null&&res.eliminated!==undefined){
    ...
    await wait(280);
    const remainAfter=aliveIndices().length;
    if(remainAfter>1 && game.track!==trackN){shrinkEffect();await wait(200);}
  }
  busy=false;
  if(mode==='online'){
